...
CMD envsubst "\$API_URL \$AUTH_URL \$SERVER_PORT \$SSL_CERT_PATH \$SSL_KEY_PATH \$SSL_CA_CERT_PATH" < /etc/nginx/templates/default.conf.template > /etc/nginx/conf.d/default.conf && nginx -g 'daemon off;'
